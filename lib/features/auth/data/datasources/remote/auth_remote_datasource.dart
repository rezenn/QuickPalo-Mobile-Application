...
class AuthRemoteDatasource implements IAuthRemoteDataSource {
  ...
  Future<AuthRegisterApiModel> register(AuthRegisterApiModel user) async {
    try {
      print('Registering user with data: ${user.toJson()}');
      ...
      if (response.data['success'] == true) {
        ...
        final registeredUser = AuthRegisterApiModel.fromJson(data);
        print('Registered user created: ${registeredUser.id}');
        ...
        if (registeredUser.id != null && registeredUser.id!.isNotEmpty) {
          ...
          print('Session saved for user: ${registeredUser.id}');
        } else {
          print('Warning: Registered user has no ID');
        }
